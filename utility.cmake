...
macro(require_git_package pkg url tag)
  ...
  cmake_parse_arguments(CHAOS_ARGS "" "VERSION" "" "${ARGN}")
  find_package(${pkg} ${CHAOS_ARGS_VERSION})
  if (NOT ${pkg_FOUND})
    message("Ready to download ${pkg}...")
    add_git_3rd_project(${pkg} ${url} ${tag} ${CHAOS_ARGS_UNPARSED_ARGUMENTS})
    find_package(${pkg} ${CHAOS_ARGS_VERSION} REQUIRED)
  endif()
...
macro(require_url_package pkg url)
  ...
  if (NOT ${pkg_FOUND})
    message("Ready to download ${pkg}...")
    add_url_3rd_project(${pkg} ${url} ${CHAOS_ARGS_UNPARSED_ARGUMENTS})
    find_package(${pkg} ${CHAOS_ARGS_VERSION} REQUIRED)
  endif()
  message("Find ${pkg} version -> ${${pkg_VERSION}}")
endmacro()
...
function(add_mac2f90 target name)
  if (NOT EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${name}.f90)
    add_custom_command(
      ...
      COMMAND maxima --userdir=${PROJECT_SOURCE_DIR}/scripts/maxima
      -b ${PROJECT_SOURCE_DIR}/scripts/maxima/${name}.mac --very-quiet
