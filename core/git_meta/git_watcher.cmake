# modified from https://raw.githubusercontent.com/andrew-hardin/cmake-git-version-tracking/master/git_watcher.cmake
# The original file is released under MIT License.
# https://raw.githubusercontent.com/andrew-hardin/cmake-git-version-tracking/master/LICENSE
# The modification allows it to support multiple git project.
...
#
#   PRE_SOURCE_CONFIGURE_FILE (REQUIRED)
#   -- The path to the file that'll be configured.
#
#   POST_SOURCE_CONFIGURE_FILE (REQUIRED)
#   -- The path to the configured PRE_SOURCE_CONFIGURE_FILE.
#
...
set(GIT_META_ROOT ${CMAKE_CURRENT_LIST_DIR})
...
macro(CHECK_REQUIRED_VARIABLE_AND_SET_ABSOLUTE var_name)
    CHECK_REQUIRED_VARIABLE(${var_name})
...
function(GitStateChangedAction)
    ...
    endforeach()
    configure_file("${PRE_SOURCE_CONFIGURE_FILE}" "${POST_SOURCE_CONFIGURE_FILE}" @ONLY)
endfunction()
...
function(CheckGit _working_dir _state_changed)
    ...
    file(SHA256 ${PRE_SOURCE_CONFIGURE_FILE} preconfig_hash)
...
function(SetupGitMonitoring)
    add_custom_target(${GIT_PROJECT_NAME}_check_git
        ALL
        DEPENDS ${PRE_SOURCE_CONFIGURE_FILE}
        BYPRODUCTS
        ...
        COMMAND
            ...
            -DGIT_PROJECT_NAME=${GIT_PROJECT_NAME}
            -DGIT_WORKING_DIR=${GIT_WORKING_DIR}
            -DGIT_EXECUTABLE=${GIT_EXECUTABLE}
            -DGIT_STATE_FILE=${GIT_STATE_FILE}
            -DPRE_SOURCE_CONFIGURE_FILE=${PRE_SOURCE_CONFIGURE_FILE}
            -DPOST_SOURCE_CONFIGURE_FILE=${POST_SOURCE_CONFIGURE_FILE}
            -DPRE_HEADER_CONFIGURE_FILE=${PRE_HEADER_CONFIGURE_FILE}
            -DPOST_HEADER_CONFIGURE_FILE=${POST_HEADER_CONFIGURE_FILE}
            -DGIT_FAIL_IF_NONZERO_EXIT=${GIT_FAIL_IF_NONZERO_EXIT}
            -DGIT_IGNORE_UNTRACKED=${GIT_IGNORE_UNTRACKED}
            -DGIT_META_OUTDIR=${GIT_META_OUTDIR}
            -P "${GIT_META_ROOT}/main.cmake")
endfunction()
...
function(Main)
    ...
    if(_BUILD_TIME_CHECK_GIT)
        ...
        if(changed OR NOT EXISTS "${POST_SOURCE_CONFIGURE_FILE}")
